...
class Q11Codex0ReplicatedKernel {
private:
...
public:
    Q11Codex0ReplicatedKernel(
        const int *date, const int *disc, const int *quant, const int *price, uint64_t *res)
        : lo_orderdate(date), lo_discount(disc), lo_quantity(quant), lo_extendedprice(price), agg_result(res)
    {}

    void operator()(sycl::id<1> idx) const {
        auto i = idx[0];
        
        // Replicating Codex0 modular execution internals sequentially:
        bool f = true;

        // Selection 1 & 2: d_year mapping + filter (Simplified for Q1.1 date constants)
        f = logical(AND, f, compare(GE, lo_orderdate[i], 19930101));
        f = logical(AND, f, compare(LE, lo_orderdate[i], 19931231));

        // Selection 3 & 4: lo_discount filter
        f = logical(AND, f, compare(GE, lo_discount[i], 1));
        f = logical(AND, f, compare(LE, lo_discount[i], 3));

        // Selection 5: lo_quantity filter
        f = logical(AND, f, compare(LT, lo_quantity[i], 25));

        // Projection: Calculation of (extendedprice * discount)
        uint64_t proj_val = 0;
        if (f) {
            proj_val = element_operation((uint64_t)lo_extendedprice[i], (uint64_t)lo_discount[i], BinaryOp::Multiply);
        }

        // Aggregation: Explicit atomic update
        if (f) {
            sycl::atomic_ref<
                uint64_t,
...
int main(int argc, char** argv) {
    ...
    auto run_kernel = [&]() {
        Q11Codex0ReplicatedKernel kernel(d_date, d_disc, d_quant, d_price, d_res);
