...
class Q21Codex0ReplicatedKernel {
private:
    ...
    const size_t n_part, n_supp;

public:
    Q21Codex0ReplicatedKernel(
        const int *date, const int *part, const int *supp, const int *rev,
        const bool *pf, const bool *sf, const int *pb, const int *dym,
        uint64_t *agg, unsigned *flags, int *ry, int *rb, size_t np, size_t ns)
    ...
    void operator()(sycl::id<1> idx) const {
        ...
        // Reproducing Codex0 Modular Loop Internals Sequence
        // 1. Join Part & Filter
        int pkey = lo_part[i];
        bool p_in_bounds = compare(GE, pkey, (int)0) && compare(LE, pkey, (int)n_part);
        f = logical(AND, f, p_in_bounds);
        if (f) f = logical(AND, f, p_filter[pkey]);

        // 2. Join Supplier & Filter
        int skey = lo_supp[i];
        bool s_in_bounds = compare(GE, skey, (int)0) && compare(LE, skey, (int)n_supp);
        f = logical(AND, f, s_in_bounds);
        if (f) f = logical(AND, f, s_filter[skey]);

        // 3. Join Date
        int dkey = lo_date[i];
        bool d_in_bounds = compare(GE, dkey, D_MIN) && compare(LE, dkey, D_MAX);
        f = logical(AND, f, d_in_bounds);

        if (f) {
            int year = d_year_map[dkey - D_MIN];
            int brand = p_brand[pkey];
...
int main(int argc, char** argv) {
    ...
    auto run_kernel = [&]() {
        Q21Codex0ReplicatedKernel kernel(d_lo_date, d_lo_part, d_lo_supp, d_lo_rev, d_p_filter, d_s_filter, d_p_brand, d_year_map, d_res_agg, d_res_flags, d_res_year, d_res_brand, n_part, n_supp);
        q.parallel_for(sycl::range<1>(n_fact), kernel).wait();
    };
