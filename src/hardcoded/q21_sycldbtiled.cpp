 ...
 * 
 * Replaces simple flat data execution with block-striped unrolled tiles 
 * (ITEMS_PER_THREAD * BLOCK_THREADS) to replicate Mordred's explicit
 * fetch scheduling within AdaptiveCpp's compiler optimization paths.
 */
...
class Q21TiledKernel {
private:
    const int *lo_date, *lo_part, *lo_supp, *lo_revenue;
    ...
    const size_t n_part, n_supp, n_fact;

public:
    Q21TiledKernel(
        const int *date, const int *part, const int *supp, const int *rev,
        const bool *pf, const bool *sf, const int *pb, const int *dym,
        uint64_t *agg, unsigned *flags, int *ry, int *rb, size_t np, size_t ns, size_t nf)
    ...
    void operator()(sycl::nd_item<1> item) const {
        ...
        if (tile_offset + num_tile_items > n_fact) {
            num_tile_items = n_fact - tile_offset;
        }

        int items_part[ITEMS_PER_THREAD];
        int items_supp[ITEMS_PER_THREAD];
        int items_date[ITEMS_PER_THREAD];
        ...
        for(int i=0; i<ITEMS_PER_THREAD; ++i) {
            ...
            if (idx < tile_offset + num_tile_items) {
                items_part[i] = lo_part[idx];
                bool p_in_bounds = compare(GE, items_part[i], (int)0) && compare(LE, items_part[i], (int)n_part);
                f[i] = logical(AND, f[i], p_in_bounds);
                if (f[i]) f[i] = logical(AND, f[i], p_filter[items_part[i]]);
            } else { f[i] = false; }
        }

        // 2. Join Supplier & Filter
        #pragma unroll
        for(int i=0; i<ITEMS_PER_THREAD; ++i) {
            int idx = tile_offset + i * BLOCK_THREADS + local_id;
            if (idx < tile_offset + num_tile_items && f[i]) {
                items_supp[i] = lo_supp[idx];
                bool s_in_bounds = compare(GE, items_supp[i], (int)0) && compare(LE, items_supp[i], (int)n_supp);
                f[i] = logical(AND, f[i], s_in_bounds);
                if (f[i]) f[i] = logical(AND, f[i], s_filter[items_supp[i]]);
            }
        }
        ...
        for(int i=0; i<ITEMS_PER_THREAD; ++i) {
            int idx = tile_offset + i * BLOCK_THREADS + local_id;
            if (idx < tile_offset + num_tile_items && f[i]) {
                items_date[i] = lo_date[idx];
                bool d_in_bounds = compare(GE, items_date[i], D_MIN) && compare(LE, items_date[i], D_MAX);
                f[i] = logical(AND, f[i], d_in_bounds);
            }
        }

        // 4. Output logic & GroupBy Aggregation Protocols
        #pragma unroll
        for(int i=0; i<ITEMS_PER_THREAD; ++i) {
            int idx = tile_offset + i * BLOCK_THREADS + local_id;
            if (idx < tile_offset + num_tile_items && f[i]) {
                ...
                int year = d_year_map[items_date[i] - D_MIN];
                int brand = p_brand[items_part[i]];
                ...
                if (flag_obj.exchange(1) == 0) {
                    res_year[bucket] = year;
                    res_brand[bucket] = brand;
                }
                ...
                sum_obj.fetch_add((uint64_t)items_revenue[i]);
            }
        }
    }
};
...
int main(int argc, char** argv) {
    ...
    size_t num_blocks = (n_fact + (BLOCK_THREADS * ITEMS_PER_THREAD) - 1) / (BLOCK_THREADS * ITEMS_PER_THREAD);
    ...
    auto run_kernel = [&]() {
        Q21TiledKernel<BLOCK_THREADS, ITEMS_PER_THREAD> kernel(d_lo_date, d_lo_part, d_lo_supp, d_lo_rev, d_p_filter, d_s_filter, d_p_brand, d_year_map, d_res_agg, d_res_flags, d_res_year, d_res_brand, n_part, n_supp, n_fact);
