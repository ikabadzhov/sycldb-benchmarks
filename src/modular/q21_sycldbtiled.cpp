...
struct Q21JoinTiledKernel {
    const int *lo_part, *lo_supp, *lo_date; const bool *p_filter, *s_filter;
    bool *mask; size_t n_part, n_supp, n_fact;
    void operator()(sycl::nd_item<1> item) const {
        int tile_offset = item.get_group(0) * (BLOCK_THREADS * ITEMS_PER_THREAD);
        int num_tile_items = (BLOCK_THREADS * ITEMS_PER_THREAD);
        if (tile_offset + num_tile_items > n_fact) num_tile_items = n_fact - tile_offset;
        #pragma unroll
        for(int i=0; i<ITEMS_PER_THREAD; ++i) {
            int idx = tile_offset + i * BLOCK_THREADS + item.get_local_id(0);
            if (idx < tile_offset + num_tile_items) {
                int pk = lo_part[idx], sk = lo_supp[idx], dk = lo_date[idx];
                bool f = (pk >= 0 && pk < (int)n_part && p_filter[pk]);
                f &= (sk >= 0 && sk < (int)n_supp && s_filter[sk]);
...
template <int BLOCK_THREADS, int ITEMS_PER_THREAD>
struct Q21AggregationTiledKernel {
    const int *lo_date, *lo_part, *lo_revenue, *d_year_map, *p_brand;
    const bool *mask; uint64_t *agg_result; unsigned *result_flags; int *res_year, *res_brand; size_t n_fact;
    void operator()(sycl::nd_item<1> item) const {
        int tile_offset = item.get_group(0) * (BLOCK_THREADS * ITEMS_PER_THREAD);
        int num_tile_items = (BLOCK_THREADS * ITEMS_PER_THREAD);
        if (tile_offset + num_tile_items > n_fact) num_tile_items = n_fact - tile_offset;
        ...
        for(int i=0; i<ITEMS_PER_THREAD; ++i) {
            ...
            if (idx < tile_offset + num_tile_items && mask[idx]) {
                int year = d_year_map[lo_date[idx] - 19920101];
                int brand = p_brand[lo_part[idx]];
                ...
                sum_obj.fetch_add((uint64_t)lo_revenue[idx]);
            }
        }
    }
};
...
int main(int argc, char** argv) {
    ...
    auto run_kernel = [&]() {
        q.parallel_for(tile_range, Q21JoinTiledKernel<BLOCK_THREADS, ITEMS_PER_THREAD>{d_lo_part, d_lo_supp, d_lo_date, d_p_filter, d_s_filter, d_mask, n_part, n_supp, n_fact});
        q.parallel_for(tile_range, Q21AggregationTiledKernel<BLOCK_THREADS, ITEMS_PER_THREAD>{d_lo_date, d_lo_part, d_lo_rev, d_year_map, d_p_brand, d_mask, d_res_agg, d_res_flags, d_res_year, d_res_brand, n_fact}).wait();
    };
